...
import { LearnAllDetailsRequest, LearnAllRequest, LearnfavouriteListRequest } from "../../../modules/learn/actions";
...
const Learn = (props) => {
  ...
  const getApiRes = (item) => {
    ...
    let params = {
      ...
      category: item.id == "2" ? "Article" : item.id == '3' ? "Video" :
        item.id == '4' ? "Coach" : null,
      // offset: 1,
  ...
  const getSortRes = (item) => {
    ...
    let params = {
      endpoint: API_URL.fetchAllLearn,
      // coordinates: tempCords ? tempCords : userCords[0] !== undefined ? userCords : null,
      labels: allData.filter(val => val.id === selectedItemIndex)[0].id == "2" ? "Conflict" : allData.filter(val => val.id === selectedItemIndex)[0].id == '3' ? "Dating" :
        allData.filter(val => val.id === selectedItemIndex)[0].id == '4' ? "Marriage" : allData.filter(val => val.id === selectedItemIndex)[0].id == '5' ? "Support" : allData.filter(val => val.id === selectedItemIndex)[0].id == '6' ? "Love" : null,
      labels: item.name,
    };
    dispatch(LearnAllRequest(params))
    // console.log("PPPP+++", params)
  ...
  const endReached = () => {
    ...
    let params = {
      ...
      coordinates: tempCords ? tempCords : userCords[0] !== undefined ? userCords : null,
      category: allData.filter(val => val.id === selectedItemIndex)[0].id == "2" ? "6479bae337177e3f0a74c234" : allData.filter(val => val.id === selectedItemIndex)[0].id == '3' ? "649161e44755a8d0372968a1" :
        allData.filter(val => val.id === selectedItemIndex)[0].id == '4' ? "6482cc84dd8801bd2034316b" : null,
  ...
  const renderAllItem = ({ item, index }) => {
    return (
      <View style={{}}>
        <Pressable style={{
          paddingHorizontal: 15, paddingVertical: 20, marginHorizontal: 20, borderBottomWidth: 2, borderBottomColor: selectedItemIndex === item.id ? color._primary_orange : 'white', borderColor: color._dusty_white
...
const mapDispatchToProps = (dispatch) => ({
  ...
  LearnAllDetailsRequest: (data, navigation) => dispatch(LearnAllDetailsRequest(data, navigation)),
  LearnfavouriteListRequest: (data) => dispatch(LearnfavouriteListRequest(data)),
