import { SafeAreaView, Text, View, TouchableOpacity, FlatList, Pressable, Image, Alert } from "react-native";
...
import { removeAnswer, setAge, setGender, setSexualOrientation, answerRequest, setAnswer, setCity } from "../../../modules/SetAnswer/actions";
...
const Age = (props) => {
  ...
  const [longestRelationshipLength, setLongestRealtionshipLength] = React.useState()
  ...
  const insertData = () => {
    ...
    let params = {
      ...
      id: props?.state?.loginReducer?.UserData?._id ? props?.state?.loginReducer?.UserData?._id :
        props.state?.signupReducer?.signupSucessData?.UserData?._id ?  props.state?.signupReducer?.signupSucessData?.UserData?._id : props.state?.loginReducer?.loginData?._id ,
      navigation: () =>
        navigation.reset({
          index: 0,
          ...
          step == 4 && (
            <>
              ...
              <View style={{ flex: 1, marginTop: "8%" }}>
                ...
                />
                {relationshipStatus !== "Single" && relationshipStatus !== null &&
