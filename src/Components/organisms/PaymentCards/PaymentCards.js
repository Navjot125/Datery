...
import { CardAllDetailsRequest, CardAllDetailsSuccess, CardAllRequest, CardfavouriteListRequest, CardremoveFavouriteRequest, CardremoveGuestFavouriteRequest } from "../../../modules/addCard/actions";
...
const PaymentCards = (props) => {
  ...
  const getCardRes = () => {
    const token = props?.state?.loginReducer?.userToken ? props?.state?.loginReducer?.userToken :
        ...
        <View style={[styles.card]}>
          ...
          >
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Image source={require("../../../assets/images/VISA.png")} style={{ width: 47, height: 28 }} />
              {item.default && (
              <View style={{ backgroundColor: color._primary_orange, justifyContent: 'center', right: "-50%", borderTopLeftRadius: 20, borderBottomLeftRadius: 20, paddingHorizontal: 20 }}>
                <Text style={{ fontSize: 12, lineHeight: 20, color: color._white, textAlign: 'center' }}>Default</Text>
              </View>
              )}
            </View>
            <View
            ...
            >
              ...
              <View>
                ...
                <Text style={styles.cardValue}>{item.exp_month}{"/"}{item.exp_year} </Text>
              </View>
            </View>
          </ImageBackground>
        </View>
  ...
  const renderHiddenItem = (dataItem) => (
    <View style={styles.rowBack}>
      ...
      >
        <Image style={{ height: 20, width: 20 }} source={require('../../../assets/images/editCard.png')} />
...
const mapDispatchToProps = (dispatch) => ({
  ...
  CardAllDetailsRequest: (data, navigation) => dispatch(CardAllDetailsRequest(data, navigation)),
  CardAllDetailsSuccess: (data, navigation) => dispatch(CardAllDetailsSuccess(data, navigation)),
  ...
  CardremoveFavouriteRequest: (navigation) => dispatch(CardremoveFavouriteRequest(navigation)),
  CardremoveGuestFavouriteRequest: (data) => dispatch(CardremoveGuestFavouriteRequest(data))
});
