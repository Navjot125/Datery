...
const AllMap = (props) => {
    ...
    const focusOnMarker = (marker) => {
        const region = {
            ...
            latitude: marker?.viewableItems[0]?.item?.locationCordinates?.coordinates[1],
            longitude: marker?.viewableItems[0]?.item?.locationCordinates?.coordinates[0],
            latitudeDelta: 0.1,
            longitudeDelta: 0.1,
        };
    ...
    const isItemInViewable = (item) => {
        return viewableItems.findIndex((viewableItem) => viewableItem.index === item.index) !== -1;
    };
    ...
    const Item = ({ item }) => (
        ...
        >
            <Atom.Button
                title={item.name}
                onPress={() => { props.setFilter(item.name), console.log('IIIII+++', item.name); }}
                style={
                    item.selected
                        ? { height: 45 }
                        : {
                                ...
                                dataa.locationCordinates ?
                                    (
                                        <Marker
                                            ...
                                            coordinate={{ latitude: dataa?.locationCordinates?.coordinates[0], longitude: dataa?.locationCordinates?.coordinates[1] }}
                                            title={dataa.title}
                                        >
                                            {/* {console.log(dataa?.locationCordinates?.coordinates[1], dataa?.locationCordinates?.coordinates[0])} */}
                                            <View style={{
                                                height: 70, width: 60, justifyContent: 'center',
                                            }} >
                                                <Image resizeMode='cover' style={{ height: 45, width: 35 }}
